...
export default function MektupPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data } = await supabase.from("letter").select("*").single();
  ...
  async function handleSave() {
    setSaving(true);
    const { data } = await supabase.from("letter").select("id").single();
    
    if (data) {
      await supabase.from("letter").update({ content, updated_at: new Date().toISOString() }).eq("id", data.id);
    } else {
      await supabase.from("letter").insert({ content });
    }
    ...
    <main style={{ minHeight: "100vh", backgroundColor: "#0f1117", padding: "32px 24px" }}>
      <div style={{ maxWidth: "700px", margin: "0 auto" }}>
        ...
        <p style={{ color: "rgba(240,237,230,0.4)", fontFamily: "system-ui", fontSize: "13px", marginBottom: "32px" }}>
          "Sana Yazdığım Yıldız" kartında görünür. Paragraf aralarına boş satır bırak.
        </p>

        <textarea
          value={content}
          onChange={(e) => setContent(e.target.value)}
          style={{ width: "100%", minHeight: "400px", padding: "16px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", color: "#F0EDE6", fontFamily: "Georgia, serif", fontSize: "15px", outline: "none", resize: "vertical", lineHeight: 1.8, boxSizing: "border-box" }}
        />
